...
def test(params, infos):
    ...
    save_path = os.path.join(workdir,'xiaohaungji_model/s2ss_chatbot_anti.ckpt')
...
def main(infos):
    ...
    # text = test(json.load(open('params.json')), infos)
    """xiaohaungji语料"""
    text = test(json.load(open(os.path.join(workdir,'xiaohaungji_model/params.json'))), infos)
